...
6. ops/ starters
7. artifacts/ starter
8. issues/ starter (with issue template)
...
## 7. artifacts/ starter (private repo)

artifacts/README.md:
```markdown
# artifacts/ — research, findings, and instructions (PRIVATE)

Inputs that feed project and milestone definition: source material Claude or
the user gathered while scoping this project — competitor research, feasibility
findings, API/library evaluations, meeting notes, brainstorm dumps, reference
instructions to follow. Not the plan itself (that's the milestone files) and
not a decision log (that's JOURNAL.md) — this is the raw and processed
material a decision or milestone was built from.

Empty at scaffold time. `/documents:plan-project` and later planning sessions
add files here as research happens; name them by topic
(`<topic>-research.md`, `<topic>-findings.md`), not by date or session.
```

## 8. issues/ starter (private repo)

issues/README.md:
```markdown
# issues/ — bugs and incidents (PRIVATE)

One file per bug or incident, discovered during development or after
shipping (production/post-release). Not for planned work or milestone
tasks — those live in milestones/. Use `issues/NN-slug.md`, copied from
`issues/_template.md` in this directory — copy it per issue, do not link
back to it.

Empty at scaffold time.
```

issues/_template.md (copy per issue, do not edit this file in place):
```markdown
# Issue NN — Title

**Status:** OPEN | INVESTIGATING | FIXED | WONT-FIX
**Discovered:** {{date}}   **Severity:** low | medium | high | critical
**Context:** during development | post-release (production)

## Symptom
[What was observed — concrete, not a guess at cause.]

## Root cause
[Filled once known. "Unknown" is fine while investigating.]

## Fix / resolution
[What changed, or why WONT-FIX. Link commit/PR once fixed.]

## Follow-up
[Anything this surfaced that needs its own milestone or issue.]
```
